...
export class FooterComponent implements OnInit {
  ...
  constructor(private renderer: Renderer2,
    ...
    this.router.events.subscribe((event: Event) => {
      ...
      if (event instanceof NavigationEnd) {
        if ((this.router.url === '/chats' || this.router.url === '/chats/static-chat' || this.router.url === '/email') &&
        footerElement.item(0)) {
  ...
  ngOnInit() {
    ...
    } else if ((this.router.url.indexOf('chats') >= 0) || (this.router.url.indexOf('static-chat') >= 0) ||
    (this.router.url.indexOf('email') >= 0)) {
      this.showFooter = false;
